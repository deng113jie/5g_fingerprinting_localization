...
void NRPhyUe::handleAirFrame(cMessage* msg)
{
    ...
    rssi /= rssiV.size();
    // ltenic               ue
    if(getParentModule()->getParentModule()->findSubmodule("mobility") != -1)
    {
        MobilityBase *mob;
        mob = check_and_cast<MobilityBase *>(getParentModule()->getParentModule()->getSubmodule("mobility"));
        EV << "gNB "<<masterId_ << " UE " << nodeId_ <<" on band " << lteInfo->getCarrierFrequency() << " time "<< simTime() << " LocationRSSI " <<  mob ->lastPosition <<" with RSSI "<< rssi << endl;
    }
    /* updates end */
